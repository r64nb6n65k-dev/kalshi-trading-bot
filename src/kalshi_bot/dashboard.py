...
def record_model_snapshot(**x: Any) -> None:
    ...
    keys = [
        "ticker", "seconds_left", "eth_price", "target_price", "separation",
...
HTML = '''<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Coby Ethereum Bot</title><style>body{font-family:system-ui;background:#0d1117;color:#e6edf3;margin:18px}.g{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:10px}.c{background:#161b22;border:1px solid #30363d;border-radius:12px;padding:12px;margin-bottom:8px}.v{font-size:24px;font-weight:700}.s{color:#8b949e;font-size:12px}a{color:#58a6ff}.green{color:#3fb950}.red{color:#f85149}table{width:100%;border-collapse:collapse;font-size:13px}td,th{padding:8px;border-bottom:1px solid #30363d;text-align:left}</style><h1>Coby Ethereum Bot</h1><div><a href="/download/trades.csv">Download trades CSV</a> ÃÂ· <a href="/download/model.csv">Download model data CSV</a></div><h2>Live Model</h2><div class=g id=live></div><h2>Performance</h2><div class=g id=stats></div><h2>Open Position</h2><div id=open></div><h2>Recent Model Decisions</h2><div id=decisions></div><h2>Recent Trades</h2><div id=trades></div><script>const f=(x,n=1)=>x==null?'Ã¢ÂÂ':Number(x).toFixed(n),card=(k,v)=>`<div class=c><div class=s>${k}</div><div class=v>${v}</div></div>`;async function r(){try{let d=await(await fetch('/api/data',{cache:'no-store'})).json(),x=d.latest||{},s=d.stats;live.innerHTML=card('Decision',x.decision||'WAIT')+card('Reason',x.reason||'Ã¢ÂÂ')+card('Ethereum','$'+f(x.eth_price,2))+card('Target','$'+f(x.target_price,2))+card('Separation','$'+f(x.separation,2))+card('Seconds left',f(x.seconds_left,0))+card('Model YES',f(x.model_yes,1)+'%')+card('Model NO',f(x.model_no,1)+'%')+card('YES edge',f(x.edge_yes,1)+'ÃÂ¢')+card('NO edge',f(x.edge_no,1)+'ÃÂ¢')+card('15s momentum','$'+f(x.momentum_15,2))+card('60s momentum','$'+f(x.momentum_60,2))+card('Volatility',f(x.volatility,3));stats.innerHTML=card('P&L',(s.total_pnl_cents>=0?'+':'')+'$'+f(s.total_pnl_cents/100,2))+card('Win rate',f(s.win_rate,1)+'%')+card('Trades',s.total_trades)+card('Record',s.wins+'-'+s.losses);open.innerHTML=d.open_positions.length?d.open_positions.map(p=>`<div class=c>${p.side.toUpperCase()} @ ${p.entry_price}ÃÂ¢ ÃÂ· ${p.count} contracts ÃÂ· ${p.execution_mode||'unknown'}<br><span class=s>Stop ${p.stop_price}ÃÂ¢ ÃÂ· TP ${p.take_profit}ÃÂ¢ ÃÂ· ${p.ticker}</span></div>`).join(''):'<div class=c>No open position</div>';decisions.innerHTML=d.snapshots.slice(0,12).map(q=>`<div class=c><b>${q.decision||'WAIT'}</b> ÃÂ· ${q.reason||'Ã¢ÂÂ'}<br><span class=s>ETH $${f(q.eth_price,2)} / target $${f(q.target_price,2)} ÃÂ· sep $${f(q.separation,2)} ÃÂ· ${f(q.seconds_left,0)}s ÃÂ· YES ${f(q.model_yes,1)}% ÃÂ· edge ${f(q.edge_yes,1)}ÃÂ¢</span></div>`).join('');trades.innerHTML='<table><tr><th>Side</th><th>Entry</th><th>Exit</th><th>P&L</th><th>Reason</th></tr>'+d.trades.slice(0,25).map(t=>`<tr><td>${t.side}</td><td>${t.entry_price}ÃÂ¢</td><td>${t.exit_price}ÃÂ¢</td><td>${(t.pnl_cents/100).toFixed(2)}</td><td>${t.reason}<br><span class=s>${t.execution_mode||'unknown'} ÃÂ· stop ${t.stop_price??'Ã¢ÂÂ'}ÃÂ¢ ÃÂ· TP ${t.take_profit??'Ã¢ÂÂ'}ÃÂ¢</span></td></tr>`).join('')+'</table>'}catch(e){console.error(e)}}r();setInterval(r,2000)</script>'''
