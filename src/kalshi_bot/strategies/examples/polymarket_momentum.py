...
class PolymarketMomentumStrategy:
    ...
    def _probability(
        ...
        if not reference_ticks:
            return None, 0.50, "NO_CHAINLINK_TWAP_HISTORY"

        latest_ref = reference_ticks[-1]
        if now - latest_ref.timestamp.timestamp() > 15:
            return None, 0.50, "STALE_CHAINLINK_TWAP"
        ...
        if now - history_start < self.minimum_history:
            return None, 0.50, "INSUFFICIENT_PRICE_HISTORY"
        ...
        return yes_probability, persistence, detail
    ...
    def evaluate(
        ...
        yes_probability, persistence, detail = self._probability(
            target,
            now,
            seconds_left,
            ticks,
        ...
        if ask is None or not 1 <= ask <= 99:
            ...
            return None
        # Two days of live data: entries below 65c lost money on both days
        # (32.1% win/-$4.36 over 53 trades, then 30.0% win/-$1.27 over 10
        # trades) regardless of how the probability gate below was set --
        # cheap entries underperform independent of the modeled edge. Hard
        # floor rather than relying solely on the probability curve.
        proven_entry_floor = max(self.minimum_entry_price, 65)
        if ask < proven_entry_floor:
            self._snapshot_retryable(
                listing,
                seconds_left,
                target,
                f"BELOW_PROVEN_ENTRY_FLOOR | ask={ask}c floor={proven_entry_floor}c",
            )
            return None
        ...
        # SOL lost money on both days recorded so far (-$3.08, then -$1.74)
        # while every other coin flipped sign between days -- not noise,
        # a repeatable pattern. Extra margin rather than an outright ban:
        # still tradeable, just needs materially stronger conviction.
        coin_margin = 0.06 if listing.slug.startswith("sol-") else 0.0
        required_with_margin = (
            required_probability + self.minimum_probability_margin + coin_margin
        )
        ...
        # Chop filter: persistence is the fraction of recent (45s) tick-to-
        # tick moves that actually agreed with the forecast direction --
        # already computed above but previously only nudged the probability
        # by +/-0.08, never gated. Live evidence: 7 trades in one 32-minute
        # stretch all reversed within 1-2 seconds of entry, across BTC, ETH,
        # SOL, and XRP simultaneously -- a whipsaw regime, not 7 independent
        # bad reads. A true trend has most recent moves agreeing with it;
        # 0.60 means at least 3-in-5 recent moves were aligned. Below that,
        # the "confirmed" direction is barely more common than its opposite.
        if persistence < 0.60:
            self._snapshot_retryable(
                listing,
                seconds_left,
                target,
                (
                    f"CHOPPY_PRICE_ACTION | predicted={side.value} persistence={persistence:.2f} "
                    f"required=0.60 p_side={model_probability:.3f} | {detail}"
                ),
            )
            return None
        ...
        self._last_retry_reason.pop(listing.slug, None)
        # Claim this ticker the instant the signal fires -- not when
        # open_position() eventually runs. Previously the self.decided
        # guard above was only satisfied by open_position(), which is
        # called by the executor after order placement/fill confirmation.
        # If that takes even 1-2s, repeated evaluate() polls in the
        # meantime each saw an "undecided" ticker and emitted another
        # BUY signal -- observed live as 6 separate BUY_NO fills on one
        # ticker in 22s (73c down to 5c) while price collapsed, ballooning
        # a 2-contract position into 15. Marking decided here closes that
        # window; open_position() still runs normally once the fill
        # confirms and is a no-op against this guard either way.
        self.decided.add(listing.slug)
        self._snapshot(
            listing,
            seconds_left,
