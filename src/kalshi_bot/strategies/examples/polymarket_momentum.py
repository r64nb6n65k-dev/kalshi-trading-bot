...
STRATEGY_VERSION = "poly-5m-target-hold-v12"
...
class PolymarketMomentumStrategy:
    ...
    def __init__(
        ...
        *,
        ...
        efficiency_reference: float = 0.20,
        ...
        minimum_entry_seconds: float = 180.0,
        ...
        self.efficiency_reference = max(0.01, min(1.0, efficiency_reference))
        ...
        # The historical trade files were most consistent when the underlying
        # had established one side of the opening target early in the market.
        # Do not start a new position after the final two minutes begin.
        self.minimum_entry_seconds = max(0.0, minimum_entry_seconds)
    ...
    def _dynamic_entry_ceiling(self, detail: str) -> int:
        """Keep contract pricing out of the directional prediction."""
    ...
    def _signal(
        ...
        efficiency_penalty = 8.0 * max(
            0.0,
            (self.efficiency_reference - efficiency) / self.efficiency_reference,
        )
        ...
        # Predict only that the underlying will remain on the side of the
        # opening price it has held since the market opened.  A target crossing
        # means the market has already demonstrated that the side is unstable.
        if crossings:
            return None, f"TARGET_ALREADY_CROSSED | crossings={crossings} | {detail}"

        # A price moving back toward the target is not a hold signal, even if it
        # remains barely on the same side at this instant.
        aligned_15_bps = direction * very_short_bps
        if aligned_15_bps < 0.0:
            return None, (
                "PRICE_FADING_TOWARD_TARGET | "
                f"aligned_momentum_15_bps={aligned_15_bps:+.2f} | {detail}"
            )

        # The opening target alone owns direction. Contract quotes are checked
        # later only to enforce the 50c-65c execution band.
    ...
    def evaluate(
        ...
        if seconds_left > decision:
            return None
        if seconds_left < self.minimum_entry_seconds:
            ...
            self._snapshot(
                ...
                target,
                "SKIP",
                "TARGET_HOLD_ENTRY_WINDOW_EXPIRED | "
                f"minimum_entry_seconds={self.minimum_entry_seconds:.0f}",
            )
            return None
        ...
        if side is not previous_side:
            ...
            # Confirmation requires a genuinely new tick that does not move
            # against the selected direction. Adverse movement starts a fresh
            # sequence instead of validating a stale burst; flat remains neutral.
            if confirmation_move_bps < 0.0:
