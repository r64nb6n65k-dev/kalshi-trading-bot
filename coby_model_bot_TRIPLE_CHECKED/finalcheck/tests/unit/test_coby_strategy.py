...
def test_confirmation_resets_after_low_confidence_interruption() -> None:
    strategy = CobyStrategy(
        minimum_history_seconds=30,
        required_confirmations=3,
    )
    qualified = _ctx([110.0 + index for index in range(61)])
    assert strategy.on_market_data(qualified) == []
    assert strategy.on_market_data(qualified) == []

    # Make the same otherwise-qualified reading fail confidence. A true
    # consecutive-confirmation rule must reset here.
    strategy.min_confidence = 0.9999
    assert strategy.on_market_data(qualified) == []

    strategy.min_confidence = 0.72
    assert strategy.on_market_data(qualified) == []
    assert strategy._confirmation_count_by_ticker[qualified.market.ticker] == 1


def test_recovered_low_price_position_does_not_instant_stop_at_79() -> None:
    strategy = CobyStrategy()
    ticker = "KXBTC15M-RECOVER"
    market = Market(
        ticker=ticker,
        close_time=_close_time(),
        yes_bid=78,
    ...
    # No saved entry price is available. Recovery should not treat 79c as an
    # absolute stop and immediately liquidate a potentially valid sub-79c entry.
    assert strategy.on_market_data(ctx) == []
