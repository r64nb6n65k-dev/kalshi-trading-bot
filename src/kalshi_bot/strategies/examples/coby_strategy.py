...
class CobyStrategy(Strategy):
    ...
    def __init__(self, **params: Any) -> None:
        ...
        self.chaos_efficiency_ceiling = float(params.get("chaos_efficiency_ceiling", 0.22))
        self.chaos_reversal_ratio = float(params.get("chaos_reversal_ratio", 0.55))
        self.chaos_min_travel = float(params.get("chaos_min_travel", 12.00))
    ...
    def on_market_data(self, ctx: StrategyContext) -> list[OrderRequest]:
        ...
        if count > 0:
            ...
            if ticks and m.floor_strike is not None:
                latest_tick = ticks[-1]
                cutoff = latest_tick.timestamp.timestamp() - 
