...
class PolymarketMomentumStrategy:
    ...
    def __init__(
        ...
        *,
        ...
        # Preserve the current confirmed stop behavior while restoring the
        # v11 entry/prediction logic.
    ...
    def _dynamic_entry_ceiling(self, detail: str) -> int:
        """Reject weak chart confidence and enforce the hard strategy price cap."""
    ...
    def evaluate(
        ...
        if now < self._loss_cooldown_until:
            self._snapshot_retryable(
                ...
                seconds_left,
                target,
                f"LOSS_CLUSTER_COOLDOWN | remaining_seconds="
                f"{self._loss_cooldown_until - now:.0f}",
            )
            return None
        if target is None:
            self._snapshot_retryable(
    ...
    def close_position(self, slug: str, exit_price: int, reason: str, now: float) -> None:
        ...
        if pnl < 0:
            ...
            if self._consecutive_losses >= self.loss_cluster_threshold:
                self._loss_cooldown_until = max(
                    self._loss_cooldown_until,
                    now + self.loss_cooldown_seconds,
                )
                self._consecutive_losses = 0
                logger.warning(
                    "LOSS CLUSTER COOLDOWN STARTED | seconds=%.0f",
    ...
    def stop_loss_exit_price(
        ...
        """Trigger only after the same confirmed adverse sequence as before."""
        position = self.positions.get(listing.slug)
        if position is None:
            return None
        bid = listing.yes_bid if position.side is Side.YES else listing.no_bid
